...
- Chase, orbit, cockpit and split views show one trajectory. The physical model has no occupant. Force vectors and the flown path can be toggled. Aircraft motion and propeller phase derive from the sampled frame. The supplied GLB has integrated, static control surfaces; control commands remain available in the lesson telemetry. RPM responses are estimated visual proxies.
...
The aircraft is the revision-03 Extra 300 asset from `strikerdlm/3d-physiology-webxr`, with its original red/white/navy livery, single cockpit and two-blade propeller. The self-contained GLB has an 8.00 m span and 6.613816 m mesh length. Its geometry and materials are retained, with a rigid transform into CGEM body axes and an animated propeller pivot. The cockpit camera and live panel are fitted presentation details. [Asset provenance](public/models/README.md) records the pinned source, checksum, dimensions and source rights notes. The flight dynamics retain their existing Extra 300L parameters, including solo/dual loading choices.

The viewport shows loading and failure states and redraws the current lesson time when the model becomes ready. A failed model load can be retried by reloading the page. Each viewport and exporter owns and disposes its geometry, materials and embedded textures, including assets arriving after navigation or cancellation.
...
Browser tests use documented real-response fixtures isolated under `e2e/fixtures`; production does not import them. They cover playback, seeking, view selection, setup invalidation, stale responses, unavailable/error states, mobile layout, legacy trace routing and video validation/cancellation, model loading/failure/recovery and camera rendering with the imported model. The complete native flight/API/CGEM validation commands and numerical results are in the model documentation. To validate H.264 with an installed Chrome executable, set `CGEM_CHROME_PATH` for Playwright.
