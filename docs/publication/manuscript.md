...
**Methods.** A synthetic dataset of 3,240 CGEM runs (72 maneuvers × 45 pilot configurations; master seed 42) was generated. Per-target XGBoost surrogates used a two-stage classifier-then-regressor pattern for right-censored event-time targets and single-stage regressors for two continuous targets. Mondrian-stratified conformal layers (α = 0.05) per maneuver category combined homoscedastic split-conformal (four targets) and heteroscedastic Conformalized Quantile Regression for `time_to_gloc_s` (pre-registered as OSF amendment H5). A robust Mahalanobis detector with distribution-free conformal abstention provided OOD warning over a 17-dimensional feature space. Sobol and Morris sensitivity analyses were driven by the surrogate. The validation protocol was pre-registered on OSF before any test-set evaluation.

**Results.** On the held-out test split, the conformal OOD layer achieved an empirical in-envelope rate of 0.953 versus the nominal 0.95, with the conformal threshold ~3× the parametric χ² cutoff. Mondrian conformal coverage was within 4.6 percentage points of nominal on 4 of 5 surrogate targets; the heteroscedastic CQR layer raised time-to-G-LOC coverage from 0.861 to 0.972 on n = 36 event-positive test rows. Classifier AUROC was ≥ 0.996 across the three censored targets; regressor R² was 0.82–0.90 on event-positive rows and 0.94–1.00 on continuous targets; classifier expected calibration error remained ≤ 0.014. Surrogate inference takes ~50 µs per row versus ~9 ms for direct CGEM invocation.

**External validation against an archival centrifuge cohort (n = 8 pooled records, Whinnery & Forster 2013; OSF amendment H6) revealed a slow-onset bias `δ̄ = +26.6 s [95 % CI +6.3, +52.1]` at onset ≤ 0.5 G/s and good calibration at onset ≥ 1 G/s — the regime operationally relevant to fighter and aerobatic flight.**

**Conclusions.** The framework preserves the FAA-validated core while adding emulator speed, calibrated prediction intervals, OOD abstention, and global sensitivity rankings. The H6 archival finding bounds slow-onset operational use; own-centrifuge validation is deferred. The pattern generalises to any validated ODE physiological model.
...
**Reading Table 6.** The Whinnery-Forster low-fidelity curve alone achieves RMSE 3.07 s on the test split — already comparable to a single-fidelity XGBoost regressor over the same two-feature input — because the synthetic dataset is dominated by rapid-onset rows where WF2013 is well-anchored (`time_to_loc ≈ 9.10 s` for onset ≥ 1 G/s). The MF-NARGP point-estimate result is honestly negative: at n_high ∈ {20, 50} the GP discrepancy term over-fits catastrophically (RMSE 53.18 s and 56.38 s), and only at n_high = 100 does NARGP recover an RMSE (2.99 s) within 0.2 s of the XGBoost baseline. **In this regime the multi-fidelity contribution is therefore not point-estimate gain but calibrated uncertainty quantification**: at n_high = 100 the NARGP 95 % bracket covers 0.893 of test rows — within 5.7 pp of nominal — without any conformal post-hoc step. The single-fidelity XGBoost gives a competitive point estimate but no native uncertainty. The Kennedy-O'Hagan / NARGP scheme is unstable at the n_high ≤ 50 budgets that this aerospace-physiology dataset realistically supports; the methodologically appropriate successor is a multi-fidelity deep neural network (Meng & Karniadakis [27]), whose two-network linear/non-linear decomposition is documented to be robust at small high-fidelity sample sizes where Gaussian-process discrepancy terms over-fit. Adopting MF-DNN is paper-2 scope.

This result is consistent with the H6 finding (§3.7): in the rapid-onset regime where most synthetic event-positive rows live, CGEM, the surrogate, and the WF2013 analytical curve all give similar predictions; multi-fidelity coupling becomes operationally distinguishable from single-fidelity XGBoost only when (i) the high-fidelity budget is sufficient to identify the local discrepancy and (ii) calibrated uncertainty bands are required. Future work should extend this benchmark to the **slow-onset regime** (onset ≤ 0.5 G/s) where the H6 discrepancy concentrates — that is the regime in which a multi-fidelity scheme (preferably MF-DNN [27] rather than NARGP) would be expected to deliver a clear point-estimate advantage. The full benchmark JSON is at `data/results/h6/multifidelity_benchmark.json`.
...
2. **Adaptive conformal prediction for streaming cockpit deployment.** Fully Adaptive Conformal Inference (Gibbs & Candès 2022), Aggregated ACI (Zaffran et al. 2022), Scale-Free Online Gradient Descent and Strongly Adaptive Online Conformal Prediction (Bhatnagar et al. 2023) maintain calibration under online distribution shift; a streaming wrapper around the present surrogate is the natural extension when the framework is moved from off-line parametric mission planning to in-cockpit advisory. Adaptive conformal is *not* a current methodological gap of the present batch evaluation — it is a deployment-mode upgrade.

3. **Multi-fidelity deep neural network replacement of NARGP.** §3.8 reports a Kennedy-O'Hagan / NARGP discrepancy GP that is unstable at the n_high ≤ 50 high-fidelity budgets this dataset realistically supports. The methodologically appropriate successor is a multi-fidelity deep neural network (Meng & Karniadakis [27]); MF-DNN's two-network linear/non-linear decomposition is robust at small high-fidelity sample sizes where Gaussian-process discrepancy terms over-fit. Extending the §3.8 benchmark with MF-DNN — and re-running it against the slow-onset regime where the H6 discrepancy concentrates — is the next iteration.

4. **Distributional / multi-output conformal prediction** (Chernozhukov et al. 2021; Gibbs & Candès 2024) for tighter per-stratum coverage on the `time_to_gloc_s` target, beyond the heteroscedastic CQR patch reported in §3.3.
