...
External validation of the framework against archival centrifuge datasets and against own-centrifuge subjects is the subject of separate work and is not claimed in this manuscript. Methodological extensions identified during this study include heteroscedastic / quantile-regression conformal prediction for `time_to_gloc_s` (Romano et al. 2019 [15]); multi-fidelity coupling between CGEM and analytical low-fidelity G-tolerance models (Peherstorfer et al. 2018 [19]; Boileau et al. 2015 [21]); and Bayesian per-pilot calibration of physiological parameters using the surrogate as a fast forward solver. These are scoped methodological follow-ups, not subjects of the present paper.
