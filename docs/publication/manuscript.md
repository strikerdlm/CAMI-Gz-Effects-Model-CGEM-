...
3. **Multi-fidelity deep neural network coupling.** A Kennedy-O'Hagan / NARGP benchmark (implemented as `cgem_ext.surrogate.multifidelity.MultiFidelityNARGP`, using the Whinnery-Forster 2013 analytical curve as the low-fidelity source with ρ = 1 and a Matérn-5/2 GP discrepancy term) showed catastrophic over-fitting at the n_high ≤ 50 high-fidelity budgets this dataset realistically supports (RMSE 53–56 s at n_high ∈ {20, 50} vs 3 s for single-fidelity XGBoost); NARGP only recovers competitive RMSE at n_high = 100, where the single-fidelity baseline already matches it (results archived at `data/results/h6/multifidelity_benchmark.json`). The methodologically appropriate successor is a multi-fidelity deep neural network (Meng & Karniadakis [27]); MF-DNN's two-network linear/non-linear decomposition is robust at small high-fidelity sample sizes where GP discrepancy terms over-fit. Benchmarking MF-DNN in the slow-onset regime (onset ≤ 0.5 G/s) where the H6 discrepancy concentrates is paper-2 scope.
