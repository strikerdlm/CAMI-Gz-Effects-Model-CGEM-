...
The FAA's CAMI G-Effects Model (CGEM) is a validated Fortran +Gz-tolerance model that emits cerebrovascular and visual biosignals under sustained acceleration but is computationally expensive, supplies no calibrated uncertainty, and silently accepts out-of-distribution (OOD) inputs. We close these gaps with an additive ML wrapper around the validated core. 3,240 synthetic CGEM runs (72 maneuvers × 45 pilot configurations, master seed 42) train per-target XGBoost surrogates — a two-stage classifier+regressor for right-censored event-time targets and single-stage regressors for continuous targets. Mondrian-stratified conformal layers (α = 0.05) per maneuver category combine homoscedastic split-conformal on four targets with heteroscedastic Conformalized Quantile Regression on `time_to_gloc_s`. A robust Mahalanobis detector with distribution-free conformal abstention guards the 17-feature input envelope. The full protocol was OSF-pre-registered before any test-set evaluation. On the held-out test split, conformal OOD coverage was 0.953 versus the nominal 0.95; Mondrian coverage was within 4.6 pp on 4/5 targets; CQR raised `time_to_gloc_s` coverage from 0.861 to 0.972 (n = 36 event-positive). Classifier AUROC was ≥ 0.996 (ECE ≤ 0.014); regressor R² was 0.82–0.90 on censored event-positive rows and 0.94–1.00 on continuous targets. Inference takes ~50 µs per sample versus ~9 ms for direct CGEM invocation. Against an archival centrifuge cohort (Whinnery & Forster 2013, OSF amendment H6) the surrogate shows a slow-onset bias δ̄ = +26.6 s [95 % CI +6.3, +52.1] at onset ≤ 0.5 G/s but is in-bracket at onset ≥ 1 G/s, the operationally relevant fighter/aerobatic regime. The additive-wrapper pattern generalises to any validated ODE physiological model.

**Keywords.** biomedical signal processing; surrogate emulation; conformal prediction; out-of-distribution detection; physiological modelling; acceleration physiology

---

## 1. Introduction

Validated mechanistic models that generate biomedical signals — and the monitoring or advisory systems built around them — pose a recurring problem in biomedical signal processing: the models encode decades of physiological knowledge and experimental calibration, yet they are computationally expensive, supply no calibrated uncertainty on the signals they emit, and accept out-of-distribution operational inputs without warning. This paper presents a biosignal-fidelity-preserving wrapper for one such validated mechanistic-signal generator. The wrapper is an additive machine-learning layer that (i) emulates the signal at ~ 50 µs per sample versus ~ 9 ms for the underlying mechanistic solver, (ii) supplies distribution-free conformal coverage on every biosignal channel and event-time scalar the model emits, and (iii) abstains when the input is outside its operational envelope. The wrapped model is the FAA Civil Aerospace Medical Institute's CGEM, a Fortran ODE system that produces cerebral blood flow, head-level arterial pressure, retinal oxygen delivery, and visual-function indices under sustained +Gz load; the wrapper is delivered as an open Python package, a FastAPI signal-prediction service, and a Docker image. Portela, Banga & Matabuena [23] recently demonstrated this surrogate + conformal + OOD wrapping pattern on canonical biological dynamical systems; the present work extends it into a specific regulatory aerospace-physiology setting and adds three operational refinements: (i) per-stratum (Mondrian) conformal calibration over operationally meaningful maneuver categories, (ii) heteroscedastic conformal layers for long-tailed event-time targets, and (iii) an explicit input-envelope abstention layer.
